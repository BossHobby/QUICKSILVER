...
void flip_sequencer()
{
	...
	switch (flipstage )
	{
		...
		case STAGE_FLIP_LEVELMODE:
		  ...
		  if ( flipdir )	rx_override[flipindex] = (float) LEVEL_MODE_ANGLE / (float) LEVEL_MAX_ANGLE;
		  else	rx_override[flipindex] = (float) - LEVEL_MODE_ANGLE / (float) LEVEL_MAX_ANGLE;
		  if( gettime() - levelmodetime > LEVEL_MODE_TIME )	flipstage = STAGE_FLIP_EXIT;
		break;
