...
{	
  ...
  { //PAL
   ...
   {
    lastsystem = PAL;
    if ( osdsystem != PAL ) osd_setsystem(PAL);
  ...
  { //NTSC
   if ( lastsystem != NTSC )
    {
    lastsystem = NTSC;
    if ( osdsystem != NTSC ) osd_setsystem(NTSC);
		osd_clear();									 // initial screen clear off
   ...
   if ( lastsystem > 1 )
    {		
		...
		if (warning_sent < 2)								//incriments once at boot, and again the first time through main loop.  Then cleared by a incoming signal
			{															
			if (warning_sent == 1) osd_print( "NO CAMERA SIGNAL" , BLINK , SYSTEMXPOS , SYSTEMYPOS );
			while (osd_dma_status == BUSY){};
			warning_sent++;
			lastsystem = 2;
			}
...
void osd_intro(void)
{
 ...
 while (osd_dma_status == BUSY){};																			//this wait may not be necessary
}
