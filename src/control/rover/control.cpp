...
static filter_state_t rover_dterm_dynamic_filter_state;
...
static float rover_accel_response(float value) {
  const float accel = MAX(value, 0.0f);
  return accel / (accel + ROVER_STEERING_ACCEL_RESPONSE);
}

static float rover_steering_rate_factor(float accel, float min_factor) {
  return min_factor + (1.0f - min_factor) * rover_accel_response(accel);
}

static float rover_dterm_dynamic_frequency() {
  const float throttle = constrain(fabsf(state.throttle), 0.0f, 1.0f);
  const float dynamic_throttle = throttle + throttle * (1.0f - throttle);
  const float freq = mapf(dynamic_throttle, 0.0f, 1.0f, profile.filter.dterm_dynamic_min, profile.filter.dterm_dynamic_max);
  return constrain(freq, profile.filter.dterm_dynamic_min, profile.filter.dterm_dynamic_max);
}

static float rover_filter_dterm(float dterm) {
  dterm = filter_step(profile.filter.dterm[0].type, &rover_dterm_filter[0], &rover_dterm_filter_state[0], dterm);
  dterm = filter_step(profile.filter.dterm[1].type, &rover_dterm_filter[1], &rover_dterm_filter_state[1], dterm);
  dterm = filter_step(profile.filter.dterm_dynamic_type, &rover_dterm_dynamic_filter, &rover_dterm_dynamic_filter_state, dterm);
  return dterm;
}
...
// Rover uses profile.rover.pid directly, rather than the flight PID profiles.
