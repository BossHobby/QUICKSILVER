...
// for reasons unknown to mankind, betafpv decided to make the length field of the get settings response
// and _only_ the get settings response one longer than everything else, throwing off the parser.
// now we account for that stipidity with this variable.
static bool betafpv_workaround = false;
...
static void smart_audio_auto_baud() {
  ...
  if (current_percent < 70) {
    ...
    betafpv_workaround = !betafpv_workaround;
...
static bool smart_audio_set_frequency(vtx_band_t band, vtx_channel_t channel) {
  ...
  if (smart_audio_settings.mode & SA_MODE_FREQUENCY) {
    ...
    if (smart_audio_send_payload(SA_CMD_SET_FREQUENCY, payload, 2)) {
      ...
      quic_debugf("SMART_AUDIO: smart_audio_set_frequency update requested");
      return true;
    }
    return false;
  }
...
static bool smart_audio_set_power_level(vtx_power_level_t power) {
  ...
  if (smart_audio_send_payload(SA_CMD_SET_POWER, &level, 1)) {
    smart_audio_needs_update = true;
    quic_debugf("SMART_AUDIO: smart_audio_set_power_level update requested");
    return true;
  }
  return false;
}
...
static bool smart_audio_set_pit_mode(vtx_pit_mode_t pit_mode) {
  ...
  if (smart_audio_send_payload(SA_CMD_SET_MODE, &mode, 1)) {
    smart_audio_needs_update = true;
    quic_debugf("SMART_AUDIO: smart_audio_set_pit_mode update requested");
    return true;
  }
  return false;
}
