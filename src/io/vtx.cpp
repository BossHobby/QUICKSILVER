...
vtx_power_level_t vtx_power_level_index(vtx_power_table_t *power_table, uint16_t power) {
  for (uint8_t level = 0; level < VTX_POWER_LEVEL_MAX; level++) {
    if (power >= power_table->values[level] && power <= power_table->values[level]) {
      return (vtx_power_level_t)level;
    }
  }
...
void vtx_update() {
  ...
  if (profile.receiver.aux[AUX_FPV_SWITCH] <= AUX_CHANNEL_11 && vtx_settings.pit_mode != VTX_PIT_MODE_NO_SUPPORT)
    vtx_settings.pit_mode = rx_aux_on(AUX_FPV_SWITCH) ? VTX_PIT_MODE_OFF : VTX_PIT_MODE_ON;
...
void vtx_set(vtx_settings_t *vtx) {
  ...
  vtx_settings.power_level = vtx->power_level < vtx_settings.power_table.levels ? vtx->power_level : (vtx_power_level_t)(vtx_settings.power_table.levels - 1);

  vtx_settings.band = vtx->band < VTX_BAND_MAX ? vtx->band : VTX_BAND_A;
  vtx_settings.channel = vtx->channel < VTX_CHANNEL_MAX ? vtx->channel : VTX_CHANNEL_1;
