...
void msp_vtx_send_config_reply(msp_t *msp, msp_magic_t magic) {
  ...
  msp_vtx_config_t config = {
      .vtx_type = VTXDEV_MSP,
      .band = static_cast<uint8_t>(vtx_actual.band + 1),
      .channel = static_cast<uint8_t>(vtx_actual.channel + 1),
      .power = static_cast<uint8_t>(vtx_actual.power_level + 1),
      .pitmode = static_cast<uint8_t>(vtx_actual.pit_mode == VTX_PIT_MODE_ON ? 1 : 0),
      .freq_lsb = static_cast<uint8_t>(freq & 0xFF),
      .freq_msb = static_cast<uint8_t>((freq >> 8)),
      .device_is_ready = vtx_actual.protocol != VTX_PROTOCOL_INVALID,
      .low_power_disarm = 0,
      .pit_mode_freq_lsb = static_cast<uint8_t>(freq & 0xFF),
      .pit_mode_freq_msb = static_cast<uint8_t>(freq >> 8),
