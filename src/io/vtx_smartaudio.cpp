...
static void smart_audio_auto_baud() {
  ...
  if (current_percent < 70) {
    ...
    // cycle through workaround combinations
    workaround = static_cast<smart_audio_workaround_t>(workaround + 1);
    if (workaround >= SA_WORKAROUND_MAX) {
      workaround = SA_WORKAROUND_NONE;
    }

    baud_rate += direction * 50;
    quic_debugf("SMART_AUDIO: auto baud %d (%d) workaround %d", baud_rate, direction * 50, workaround);
    smart_audio_init();
  }
...
static bool smart_audio_send_payload(uint8_t cmd, const uint8_t *payload, const uint32_t size) {
  if (!serial_vtx_is_ready()) {
    return false;
  }

  const bool use_trailing_byte = (workaround == SA_WORKAROUND_TRAILING_BYTE || workaround == SA_WORKAROUND_BOTH);
  const uint8_t extra_bytes = use_trailing_byte ? 1 : 0;
  ...
  if (use_trailing_byte) {
    vtx_frame[size + 1 + SA_HEADER_SIZE] = 0x00;
  }

  smart_audio_auto_baud();

  if (!serial_vtx_send_data(vtx_frame, len)) {
    return false;
  }
...
static void smart_audio_parse_packet(vtx_status_t *actual, uint8_t cmd, uint8_t *payload, uint32_t length) {
  ...
  case SA_CMD_GET_SETTINGS_V21: {
    ...
    smart_audio_mode = mode;

    const uint8_t channel_index = mode & SA_MODE_FREQUENCY ? vtx_find_frequency_index(frequency) : channel;
    actual->band = (vtx_band_t)(channel_index / VTX_CHANNEL_MAX);
    actual->channel = (vtx_channel_t)(channel_index % VTX_CHANNEL_MAX);
    ...
    if (smart_audio_version == 2) {
      actual->power_level = (vtx_power_level_t)MIN((int)power, VTX_POWER_LEVEL_MAX - 1);
    } else {
...
static vtx_detect_status_t smart_audio_update(vtx_status_t *actual) {
  ...
  while (parser_state > PARSER_IDLE) {
    ...
    quic_debugf("SMART_AUDIO: state %d byte %x", parser_state, data);
    ...
    case PARSER_READ_LENGTH: {
      const bool use_betafpv = (workaround == SA_WORKAROUND_BETAFPV || workaround == SA_WORKAROUND_BOTH);
      length = vtx_payload[1] = data;
      vtx_payload_offset = 0;
      parser_state = length ? PARSER_READ_PAYLOAD : PARSER_READ_CRC;
      if (length && use_betafpv) {
        length--;
      }
      break;
    }
...
static bool smart_audio_set_frequency(vtx_band_t band, vtx_channel_t channel) {
#ifdef SA_USE_SET_FREQUENCY
  if (smart_audio_mode & SA_MODE_FREQUENCY) {
    const uint16_t frequency = frequency_table[band][channel];
    const uint8_t payload[2] = {
        (frequency >> 8) & 0xFF,
        frequency & 0xFF,
    };
    if (smart_audio_send_payload(SA_CMD_SET_FREQUENCY, payload, 2)) {
  ...
  const uint8_t index = static_cast<uint8_t>(band) * static_cast<uint8_t>(VTX_CHANNEL_MAX) + static_cast<uint8_t>(channel);
  if (smart_audio_send_payload(SA_CMD_SET_CHANNEL, &index, 1)) {
    smart_audio_needs_update = true;
    return true;
  }
  return false;
...
static bool smart_audio_set_power_level(vtx_power_level_t power) {
  uint8_t level = power;
  if (smart_audio_version != 2) {
  ...
  if (smart_audio_send_payload(SA_CMD_SET_POWER, &level, 1)) {
    smart_audio_needs_update = true;
    return true;
  }
  return false;
...
static bool smart_audio_set_pit_mode(vtx_pit_mode_t pit_mode) {
  ...
  if (smart_audio_send_payload(SA_CMD_SET_MODE, &mode, 1)) {
    smart_audio_needs_update = true;
    return true;
  }
  return false;
