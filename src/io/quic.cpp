...
static void get_quic(quic_t *quic, cbor_value_t *dec) {
  ...
  quic_values value = QUIC_VAL_INVALID;
  res = cbor_decode_member(dec, &value, cbor_decode_uint8_t);
  check_cbor_error(QUIC_CMD_GET);

  res = cbor_encode_member(&enc, &value, cbor_encode_uint8_t);
  check_cbor_error(QUIC_CMD_GET);
...
static void process_blackbox(quic_t *quic, cbor_value_t *dec) {
  ...
  res = cbor_decode_member(dec, &cmd, cbor_decode_uint8_t);
...
static void process_serial(quic_t *quic, cbor_value_t *dec) {
  ...
  case QUIC_SERIAL_ENABLE: {
    ...
    res = cbor_decode_member(dec, &port, cbor_decode_uint8_t);
    check_cbor_error(QUIC_CMD_SERIAL);
    ...
    res = cbor_encode_member(&enc, &port, cbor_encode_uint8_t);
    check_cbor_error(QUIC_CMD_SERIAL);
...
bool quic_process(quic_t *quic, uint8_t *data, uint32_t size) {
  ...
  if (size < (payload_size + static_cast<uint32_t>(QUIC_HEADER_LEN))) {
    return false;
  }
