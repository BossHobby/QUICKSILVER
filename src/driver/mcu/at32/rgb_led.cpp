...
void rgb_led_init() {
  ...
  const tmr_channel_select_type channel = (tmr_channel_select_type)timer_channel_val(ch);
  tmr_output_channel_config(def->instance, channel, &tim_oc_init);

  tmr_channel_value_set(def->instance, channel, 0);
  tmr_output_channel_buffer_enable(def->instance, channel, TRUE);
  tmr_channel_enable(def->instance, channel, TRUE);
