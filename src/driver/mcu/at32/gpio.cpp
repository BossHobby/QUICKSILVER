...
static const gpio_mode_type mode_map[] = {
    GPIO_MODE_OUTPUT, // GPIO_OUTPUT
    GPIO_MODE_INPUT,  // GPIO_INPUT
    GPIO_MODE_ANALOG, // GPIO_ANALOG
    GPIO_MODE_MUX,    // GPIO_ALTERNATE
};

static const gpio_output_type output_map[] = {
    GPIO_OUTPUT_PUSH_PULL,  // GPIO_PUSHPULL
    GPIO_OUTPUT_OPEN_DRAIN, // GPIO_OPENDRAIN
};

static const gpio_drive_type speed_map[] = {
    GPIO_DRIVE_STRENGTH_MODERATE, // GPIO_DRIVE_NORMAL
    GPIO_DRIVE_STRENGTH_STRONGER, // GPIO_DRIVE_HIGH
};

static const gpio_pull_type pull_map[] = {
    GPIO_PULL_NONE, // GPIO_NO_PULL
    GPIO_PULL_UP,   // GPIO_UP_PULL
    GPIO_PULL_DOWN, // GPIO_DOWN_PULL
};
...
void gpio_pin_init_af(gpio_pins_t pin, gpio_config_t config, uint8_t af) {
  ...
  gpio_pin_mux_config(gpio_pin_defs[pin].port, (gpio_pins_source_type)gpio_pin_defs[pin].pin_index, (gpio_mux_sel_type)af);
}
