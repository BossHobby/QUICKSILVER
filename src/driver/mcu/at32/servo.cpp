...
void servo_pwm_write(const float *values) {
  for (uint32_t i = 0; i < MOTOR_PIN_MAX; i++) {
    ...
    tmr_channel_value_set(def->instance, static_cast<tmr_channel_select_type>(timer_channel_val(TIMER_TAG_CH(tag))), pulse);
  }
}
...
void servo_pwm_stop() {
  ...
  for (uint32_t i = 0; i < MOTOR_PIN_MAX; i++) {
    ...
    const timer_def_t *def = &timer_defs[TIMER_TAG_TIM(tag)];
    tmr_channel_enable(def->instance, static_cast<tmr_channel_select_type>(timer_channel_val(TIMER_TAG_CH(tag))), FALSE);
    tmr_counter_enable(def->instance, FALSE);
