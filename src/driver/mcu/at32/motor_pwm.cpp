...
void motor_pwm_init() {
  ...
  for (uint32_t i = 0; i < MOTOR_PIN_MAX; i++) {
    ...
    tmr_output_channel_config(def->instance, (tmr_channel_select_type)timer_channel_val(ch), &tim_oc_init);
    tmr_channel_value_set(def->instance, (tmr_channel_select_type)timer_channel_val(ch), 0);
    tmr_output_channel_buffer_enable(def->instance, (tmr_channel_select_type)timer_channel_val(ch), TRUE);
    tmr_channel_enable(def->instance, (tmr_channel_select_type)timer_channel_val(ch), TRUE);
...
void motor_pwm_write(float *values) {
  for (uint32_t i = 0; i < MOTOR_PIN_MAX; i++) {
    ...
    tmr_channel_value_set(def->instance, (tmr_channel_select_type)timer_channel_val(TIMER_TAG_CH(tag)), pwm);
  }
}
