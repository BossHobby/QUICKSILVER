...
dma_stream_t dma_claim_unused(dma_device_t device) {
  ...
  for (uint32_t index = DMA_STREAM_INVALID + 1; index < DMA_STREAM_MAX; index++) {
    const auto stream = static_cast<dma_stream_t>(index);
...
cbor_result_t cbor_encode_target_dma_array_t(cbor_value_t *enc, const target_dma_array_t *dma) {
  cbor_result_t res = CBOR_OK;
  ...
  for (uint32_t i = 0; i < DMA_DEVICE_MAX; i++) {
    ...
    const auto device = static_cast<dma_device_t>(i);
    CBOR_CHECK_ERROR(res = cbor_encode_dma_device_t(enc, &device));
