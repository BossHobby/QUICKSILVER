...
static void dshot_init_gpio_port(dshot_gpio_port_t *port) {
  port->dma = dma_alloc(TIMER_TAG(DSHOT_TIMER, port->timer_channel), port->dma_device);

  dma_enable_rcc(port->dma);
  ...
  const dma_channel_def_t *dma = port->dma->def;

  dma_reset(dma->channel);
  dmamux_init(dma->mux, port->dma->request);
...
static void dshot_dma_setup_port(uint32_t index) {
  ...
  timer_enable_dma_request(DSHOT_TIMER, port->timer_channel, true);
}
...
void dshot_dma_isr(const dma_assigment_t *ass) {
  for (uint32_t j = 0; j < gpio_port_count; j++) {
    ...
    timer_enable_dma_request(DSHOT_TIMER, port->timer_channel, false);
