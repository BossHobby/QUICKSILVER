...
void rgb_led_init() {
  ...
  dma_init(rgb_dma->def->stream, &init);
  dmamux_init(rgb_dma->def->mux, rgb_dma->chan->request);
...
void rgb_led_send() {
  ...
  rgb_dma->def->stream->paddr = timer_channel_addr(def->instance, ch);
  rgb_dma->def->stream->maddr = (uint32_t)rgb_timer_buffer;
  rgb_dma->def->stream->dtcnt = rgb_timer_buffer_count;

  dma_interrupt_enable(rgb_dma->def->stream, DMA_FDT_INT, TRUE);

  dma_channel_enable(rgb_dma->def->stream, TRUE);
