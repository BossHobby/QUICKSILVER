...
static bool osd_update_display() {
  ...
  while (max_size > 0) {
    ...
    const uint64_t initial_dirty_mask = dirty_mask;

    while (dirty_mask && max_size > 0) {
      ...
      while (col < cols && size < max_size && (dirty_mask & (1ULL << col)) && row_start[col].attr == attr) {
      ...
      if (!osd_push_string(attr, start, row, string, size)) {
        // Push failed - dirty_mask already has the processed bits cleared,
        // but display_dirty_rows[row] still has original value.
        // Just return and retry the whole row next time.
        return false;
      }
    ...
    // If we processed some data from this row, update it
    if (dirty_mask != initial_dirty_mask) {
      display_dirty_rows[row] = dirty_mask; // Update with remaining dirty bits
      if (dirty_mask == 0) {
        row++;
      }
    }
