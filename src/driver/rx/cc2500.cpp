...
void cc2500_set_channel(uint8_t channel, uint8_t *cal_data) {
  ...
  {
    const spi_txn_segment_t segs[] = {
        spi_make_seg_const(static_cast<uint8_t>(CC2500_FSCAL3) | static_cast<uint8_t>(CC2500_WRITE_BURST)),
  ...
  {
    const spi_txn_segment_t segs[] = {
        spi_make_seg_const(static_cast<uint8_t>(CC2500_CHANNR) | static_cast<uint8_t>(CC2500_WRITE_SINGLE), channel),
    };
    spi_seg_submit(&bus, segs);
  }
...
uint8_t cc2500_read_fifo(uint8_t *result, uint8_t len) {
  return cc2500_read_multi(static_cast<uint8_t>(CC2500_FIFO) | static_cast<uint8_t>(CC2500_READ_BURST), result, len);
}

void cc2500_write_fifo(uint8_t *data, uint8_t len) {
  cc2500_strobe(CC2500_SFTX);
  cc2500_write_multi(static_cast<uint8_t>(CC2500_FIFO) | static_cast<uint8_t>(CC2500_WRITE_BURST), data, len);
...
static bool cc2500_dectect() {
  const uint8_t chipPartNum = cc2500_read_reg(static_cast<uint8_t>(CC2500_PARTNUM) | static_cast<uint8_t>(CC2500_READ_BURST)); // CC2500 read registers chip part num
  const uint8_t chipVersion = cc2500_read_reg(static_cast<uint8_t>(CC2500_VERSION) | static_cast<uint8_t>(CC2500_READ_BURST)); // CC2500 read registers chip version
