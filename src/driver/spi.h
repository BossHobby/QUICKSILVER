...
inline void spi_seg_submit_wait(spi_bus_device_t *bus, const spi_txn_segment_t (&segs)[N]) {
  spi_seg_submit_wait_ex(bus, segs, N);
}

template <size_t N>
  requires(N < SPI_TXN_SEG_MAX)
inline void spi_seg_submit_array(spi_bus_device_t *bus, const spi_txn_segment_t (&segs)[N], spi_txn_opts_t opts = {}) {
  opts.segs = segs;
  opts.seg_count = N;
  spi_seg_submit_ex(bus, opts);
}

#define spi_seg_submit(_bus, _segs, ...) \
  spi_seg_submit_array(_bus, _segs, spi_txn_opts_t{__VA_ARGS__})
