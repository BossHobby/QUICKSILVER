...
static void frsky_d16_set_rc_data() {
  uint16_t temp_channels[8] = {
      static_cast<uint16_t>(((rx_spi_packet[10] << 8) & 0xF00) | rx_spi_packet[9]),
      static_cast<uint16_t>(((rx_spi_packet[11] << 4) & 0xFF0) | (rx_spi_packet[10] >> 4)),
      static_cast<uint16_t>(((rx_spi_packet[13] << 8) & 0xF00) | rx_spi_packet[12]),
      static_cast<uint16_t>(((rx_spi_packet[14] << 4) & 0xFF0) | (rx_spi_packet[13] >> 4)),
      static_cast<uint16_t>(((rx_spi_packet[16] << 8) & 0xF00) | rx_spi_packet[15]),
      static_cast<uint16_t>(((rx_spi_packet[17] << 4) & 0xFF0) | (rx_spi_packet[16] >> 4)),
      static_cast<uint16_t>(((rx_spi_packet[19] << 8) & 0xF00) | rx_spi_packet[18]),
      static_cast<uint16_t>(((rx_spi_packet[20] << 4) & 0xFF0) | (rx_spi_packet[19] >> 4)),
  };
...
uint8_t frsky_d16_is_valid_packet(uint8_t *packet) {
  ...
  return header->length == (FRSKY_D16_PACKET_LENGTH - 3) &&
         header->tx_id[0] == profile.receiver.bind.frsky.tx_id[0] &&
         header->tx_id[1] == profile.receiver.bind.frsky.tx_id[1] &&
         (profile.receiver.bind.frsky.rx_num == 0 || header->rx_num == 0 || profile.receiver.bind.frsky.rx_num == header->rx_num);
}
