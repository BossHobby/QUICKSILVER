...
static void redpine_set_rc_data() {
  ...
  for (uint32_t i = 0; i < 4; i++) {
    ...
    rx_channels[i] = (uint16_t)(((uint32_t)(raw - 200) * 65535) / (1840 - 200));
  }

  rx_channels[4] = (rx_spi_packet[REDPINE_CHANNEL_START + 1] & 0x08) ? AUX_VALUE_MAX : 0;
  rx_channels[5] = (rx_spi_packet[REDPINE_CHANNEL_START + 2] & 0x80) ? AUX_VALUE_MAX : 0;
  rx_channels[6] = (rx_spi_packet[REDPINE_CHANNEL_START + 4] & 0x08) ? AUX_VALUE_MAX : 0;
  rx_channels[7] = (rx_spi_packet[REDPINE_CHANNEL_START + 5] & 0x80) ? AUX_VALUE_MAX : 0;
  rx_channels[8] = (rx_spi_packet[REDPINE_CHANNEL_START + 6] & 0x01) ? AUX_VALUE_MAX : 0;
  rx_channels[9] = (rx_spi_packet[REDPINE_CHANNEL_START + 6] & 0x02) ? AUX_VALUE_MAX : 0;
  rx_channels[10] = (rx_spi_packet[REDPINE_CHANNEL_START + 6] & 0x04) ? AUX_VALUE_MAX : 0;
  rx_channels[11] = (rx_spi_packet[REDPINE_CHANNEL_START + 6] & 0x08) ? AUX_VALUE_MAX : 0;
  rx_channels[12] = (rx_spi_packet[REDPINE_CHANNEL_START + 6] & 0x10) ? AUX_VALUE_MAX : 0;
  rx_channels[13] = (rx_spi_packet[REDPINE_CHANNEL_START + 6] & 0x20) ? AUX_VALUE_MAX : 0;
  rx_channels[14] = (rx_spi_packet[REDPINE_CHANNEL_START + 6] & 0x40) ? AUX_VALUE_MAX : 0;
  rx_channels[15] = (rx_spi_packet[REDPINE_CHANNEL_START + 6] & 0x80) ? AUX_VALUE_MAX : 0;
