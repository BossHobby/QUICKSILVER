...
static void rx_serial_find_protocol() {
  ...
  if ((time_millis() - protocol_detect_time) > 1000 || protocol_to_check == RX_SERIAL_PROTOCOL_INVALID) {
    protocol_to_check = (rx_serial_protocol_t)(protocol_to_check + 1); // Check the next protocol down the list.
    ...
    state.rx_status = static_cast<rx_status_t>(static_cast<int>(RX_STATUS_DETECTING) + static_cast<int>(protocol_to_check));
