...
extern const expresslrs_mod_settings_t *current_air_rate_config();
extern const expresslrs_rf_pref_params_t *current_rf_pref_params();
...
static bool elrs_process_packet() {
  ...
  case SYNC_PACKET: {
    ...
    const uint8_t new_tlm_denom = tlm_ratio_enum_to_value((expresslrs_tlm_ratio_t)telemetry_rate_index);
...
bool rx_expresslrs_check() {
  ...
  if (next_switch_mode_pending) {
    elrs_set_switch_mode((elrs_switch_mode_t)(next_switch_mode_pending - 1), current_air_rate_config()->payload_len);
