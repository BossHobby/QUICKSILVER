...
void elrs_set_rate(uint8_t index, int32_t freq, bool invert_iq, uint32_t flrc_sync_word, uint16_t flrc_crc_seed) {
  ...
  if (air_rate_config[index].radio_type == RADIO_TYPE_SX128x_FLRC) {
    ...
    sx128x_set_lora_mod_params((sx128x_lora_bandwidths_t)air_rate_config[index].bw, (sx128x_lora_spreading_factors_t)air_rate_config[index].sf, (sx128x_lora_coding_rates_t)air_rate_config[index].cr);
