...
static void print_osd_flightmode(osd_element_t *el) {
  const uint8_t flightmode_labels[5][11] = {
      {"   ACRO   "},
      {"  LEVEL   "},
      {" RACEMODE "},
      {" HORIZON  "},
...
static void osd_display_regular() {
  ...
  for (uint8_t element = 0; element < OSD_ELEMENT_MAX; element++) {
    ...
    case OSD_WATTS: {
      print_osd_watts(el);
      osd_state.element = (osd_elements_t)(osd_state.element + 1);
      break;
    }
...
void osd_display_rate_menu() {
  ...
  if (osd_menu_select_enum(14, 3, profile_current_rates()->mode, mode_labels)) {
    ...
    profile_current_rates()->mode = (rate_modes_t)new_mode;
    osd_state.screen_phase = OSD_PHASE_CLEAR;
  }
...
void osd_display() {
  ...
  case OSD_SCREEN_FLIGHT_MODES: {
    ...
    {
      // PAGE 1
      osd_menu_select_enum_adjust(4, OSD_AUTO, "ARMING", 17, (uint8_t *)&profile.receiver.aux[AUX_ARMING], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);
      osd_menu_select_enum_adjust(4, OSD_AUTO, "PREARM", 17, (uint8_t *)&profile.receiver.aux[AUX_PREARM], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);
      osd_menu_select_enum_adjust(4, OSD_AUTO, "IDLE UP", 17, (uint8_t *)&profile.receiver.aux[AUX_IDLE_UP], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);
      osd_menu_select_enum_adjust(4, OSD_AUTO, "LEVELMODE", 17, (uint8_t *)&profile.receiver.aux[AUX_LEVELMODE], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);
      osd_menu_select_enum_adjust(4, OSD_AUTO, "RACEMODE", 17, (uint8_t *)&profile.receiver.aux[AUX_RACEMODE], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);
      osd_menu_select_enum_adjust(4, OSD_AUTO, "HORIZON", 17, (uint8_t *)&profile.receiver.aux[AUX_HORIZON], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);
      osd_menu_select_enum_adjust(4, OSD_AUTO, "STICK BOOST", 17, (uint8_t *)&profile.receiver.aux[AUX_STICK_BOOST_PROFILE], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);

      // PAGE 2
      osd_menu_select_enum_adjust(4, OSD_AUTO, "BUZZER", 17, (uint8_t *)&profile.receiver.aux[AUX_BUZZER_ENABLE], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);
      osd_menu_select_enum_adjust(4, OSD_AUTO, "TURTLE", 17, (uint8_t *)&profile.receiver.aux[AUX_TURTLE], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);
      osd_menu_select_enum_adjust(4, OSD_AUTO, "MOTOR TEST", 17, (uint8_t *)&profile.receiver.aux[AUX_MOTOR_TEST], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);
      osd_menu_select_enum_adjust(4, OSD_AUTO, "FPV SWITCH", 17, (uint8_t *)&profile.receiver.aux[AUX_FPV_SWITCH], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);
      osd_menu_select_enum_adjust(4, OSD_AUTO, "BLACKBOX", 17, (uint8_t *)&profile.receiver.aux[AUX_BLACKBOX], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);
      osd_menu_select_enum_adjust(4, OSD_AUTO, "PREARM", 17, (uint8_t *)&profile.receiver.aux[AUX_PREARM], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);
      osd_menu_select_enum_adjust(4, OSD_AUTO, "OSD PROFILE", 17, (uint8_t *)&profile.receiver.aux[AUX_OSD_PROFILE], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_MAX - 1);
    }
  ...
  case OSD_SCREEN_VTX:
    ...
    if (vtx_settings.detected) {
      ...
      osd_menu_select_enum_adjust(4, 4, "BAND", 20, (uint8_t *)&vtx_settings_copy.band, band_labels, VTX_BAND_A, VTX_BAND_MAX - 1);

      const char *channel_labels[] = {"1", "2", "3", "4", "5", "6", "7", "8"};
      osd_menu_select_enum_adjust(4, 5, "CHANNEL", 20, (uint8_t *)&vtx_settings_copy.channel, channel_labels, VTX_CHANNEL_1, VTX_CHANNEL_8);
      osd_menu_select_enum_adjust(4, 6, "POWER LEVEL", 20, (uint8_t *)&vtx_settings_copy.power_level, power_level_labels, VTX_POWER_LEVEL_1, vtx_settings.power_table.levels - 1);
      ...
      if (osd_menu_select_enum(20, 7, vtx_settings_copy.pit_mode, pit_mode_labels) && vtx_settings_copy.pit_mode != VTX_PIT_MODE_NO_SUPPORT) {
        vtx_settings_copy.pit_mode = (vtx_pit_mode_t)osd_menu_adjust_int(vtx_settings_copy.pit_mode, 1, VTX_PIT_MODE_OFF, VTX_PIT_MODE_ON);
      }
  ...
  case OSD_SCREEN_PID_MODIFIER: {
    ...
    osd_menu_select_enum_adjust(2, 4, "P-TERM VOLTAGE COMP", 23, (uint8_t *)&profile.voltage.pid_voltage_compensation, modifier_state_labels, 0, 1);
    osd_menu_select_enum_adjust(2, 5, "THROTTLE D ATTENUATE", 23, (uint8_t *)&profile.pid.throttle_dterm_attenuation.tda_active, modifier_state_labels, 0, 1);
  ...
  case OSD_SCREEN_RSSI: {
    ...
    osd_menu_select_enum_adjust(4, 4, "RSSI FROM", 16, (uint8_t *)&profile.receiver.lqi_source, rssi_source_labels, RX_LQI_SOURCE_PACKET_RATE, RX_LQI_SOURCE_DIRECT);
    if (profile.receiver.lqi_source == RX_LQI_SOURCE_CHANNEL) {
      osd_menu_select_enum_adjust(4, 5, "SELECT AUX", 16, (uint8_t *)&profile.receiver.aux[AUX_RSSI], aux_channel_labels, AUX_CHANNEL_0, AUX_CHANNEL_11);
    }
