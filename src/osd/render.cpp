...
#ifdef USE_VTX
static profile_vtx_t osd_vtx_profile;
static uint8_t osd_vtx_profile_staged = 0;

static void osd_vtx_stage_reset() {
  osd_vtx_profile_staged = 0;
}

static profile_vtx_t *osd_vtx_stage() {
  if (!osd_vtx_profile_staged) {
    osd_vtx_profile = profile.vtx;
    if (vtx_actual.power_table.levels > 0) {
      osd_vtx_profile.power_table = vtx_actual.power_table;
      if (osd_vtx_profile.power_level >= osd_vtx_profile.power_table.levels) {
        osd_vtx_profile.power_level = static_cast<vtx_power_level_t>(osd_vtx_profile.power_table.levels - 1);
      }
    }
    osd_vtx_profile_staged = 1;
  }
  return &osd_vtx_profile;
}

static void osd_vtx_stage_commit() {
  if (osd_vtx_profile_staged) {
    profile.vtx = osd_vtx_profile;
  }
  osd_vtx_stage_reset();
}
#else
...
static void osd_menu_select_aux_adjust(int8_t x, int8_t y, const char *text, uint8_t channel_x, uint8_t range_x, aux_function_map_t *map) {
  ...
  if (osd_menu_select_enum(channel_x, y, map->channel, aux_channel_short_labels)) {
    map->channel = static_cast<rx_channel_t>(osd_menu_adjust_int(map->channel, 1, RX_CHANNEL_1, RX_CHANNEL_COUNT - 1));
  }
...
#ifdef USE_VTX
  if (osd_state.screen == OSD_SCREEN_VTX && screen != OSD_SCREEN_VTX) {
    osd_vtx_stage_reset();
  }
#endif
...
void osd_exit() {
#ifdef USE_VTX
  if (osd_state.screen == OSD_SCREEN_VTX) {
    osd_vtx_stage_reset();
  }
#endif
  osd_state.selection = 0;
...
static void osd_display_regular() {
  ...
  while (osd_state.element < OSD_ELEMENT_MAX && !el->active) {
    osd_state.element = static_cast<osd_elements_t>(osd_state.element + 1);
...
void osd_display() {
  ...
  case OSD_SCREEN_VTX:
    ...
    if (vtx_actual.protocol != VTX_PROTOCOL_INVALID) {
      profile_vtx_t *vtx_profile = osd_vtx_stage();
      ...
      const uint8_t power_levels = MIN(vtx_actual.power_table.levels, VTX_POWER_LEVEL_MAX);
      for (uint8_t i = 0; i < power_levels; i++) {
      ...
      if (power_levels > 0 && vtx_profile->power_level >= power_levels) {
        vtx_profile->power_level = static_cast<vtx_power_level_t>(power_levels - 1);
      }
      ...
      osd_menu_select_enum_adjust(4, 4, "BAND", 20, (uint8_t *)&vtx_profile->band, band_labels, VTX_BAND_A, VTX_BAND_MAX - 1);

      const char *channel_labels[] = {"1", "2", "3", "4", "5", "6", "7", "8"};
      osd_menu_select_enum_adjust(4, 5, "CHANNEL", 20, (uint8_t *)&vtx_profile->channel, channel_labels, VTX_CHANNEL_1, VTX_CHANNEL_8);
      if (power_levels > 0) {
        osd_menu_select_enum_adjust(4, 6, "POWER LEVEL", 20, (uint8_t *)&vtx_profile->power_level, power_level_labels, VTX_POWER_LEVEL_1, power_levels - 1);
      }

      const char *pit_mode_labels[] = {"OFF", "ON ", "N/A"};
      osd_menu_select(4, 7, "PITMODE");
      if (osd_menu_select_enum(20, 7, vtx_profile->pit_mode, pit_mode_labels) && vtx_profile->pit_mode != VTX_PIT_MODE_NO_SUPPORT) {
        vtx_profile->pit_mode = static_cast<vtx_pit_mode_t>(osd_menu_adjust_int(vtx_profile->pit_mode, 1, VTX_PIT_MODE_OFF, VTX_PIT_MODE_ON));
      }

      if (osd_menu_button(4, OSD_END, "SAVE AND EXIT")) {
        osd_vtx_stage_commit();
        osd_save_exit();
      }
      osd_menu_finish();
    } else
  ...
  case OSD_SCREEN_RSSI: {
    ...
    if (profile.receiver.lqi_source == RX_LQI_SOURCE_CHANNEL) {
      osd_menu_select_enum_adjust(4, 5, "SELECT AUX", 16, (uint8_t *)&profile.receiver.aux[AUX_RSSI].channel, aux_channel_labels, RX_CHANNEL_1, RX_CHANNEL_16);
    }
  ...
  case OSD_SCREEN_BLACKBOX_PRESETS: {
    ...
    uint32_t y = 5;
    for (uint32_t i = 0; i < blackbox_presets_count; i++, y++) {
      const blackbox_preset_t *preset = &blackbox_presets[i];
