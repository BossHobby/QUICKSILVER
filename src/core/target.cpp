...
static constexpr uint8_t pin_none_str[] = "NONE";
...
static constexpr auto make_gpio_pin_lookup() {
  struct {
    gpio_pins_t t[GPIO_PORT_MAX * GPIO_PIN_MAX];
  } r = {};
#define GPIO_AF(pin, af, tag)
#define GPIO_PIN(_port, _num) r.t[GPIO_PORT##_port * GPIO_PIN_MAX + _num] = PIN_##_port##_num;
...
static constexpr auto gpio_pin_lookup = make_gpio_pin_lookup();

static constexpr auto make_gpio_port_lookup() {
  struct {
    uint8_t t[PINS_MAX];
  } r = {};
#define GPIO_AF(pin, af, tag)
#define GPIO_PIN(_port, _num) r.t[PIN_##_port##_num] = GPIO_PORT##_port;
#include "gpio_pins.in"
#undef GPIO_PIN
#undef GPIO_AF
  return r;
...
static constexpr auto gpio_port_lookup = make_gpio_port_lookup();

static gpio_pins_t gpio_pin_from_port_pin(uint8_t port, uint8_t pin) {
  if (port >= GPIO_PORT_MAX || pin >= GPIO_PIN_MAX) {
    return PIN_NONE;
  }
  return gpio_pin_lookup.t[port * GPIO_PIN_MAX + pin];
}
...
cbor_result_t cbor_encode_gpio_pins_t(cbor_value_t *enc, const gpio_pins_t *t) {
  ...
  if (*t != PIN_NONE) {
    const uint8_t port = gpio_port_lookup.t[*t];
...
cbor_result_t cbor_decode_gpio_pins_t(cbor_value_t *dec, gpio_pins_t *t) {
  ...
  uint8_t port = buf[1] - 'A';
  uint8_t pin_num;
  if (size == 4) {
    pin_num = (buf[2] - '0') * 10 + (buf[3] - '0');
  } else {
    pin_num = buf[2] - '0';
  }

  *t = gpio_pin_from_port_pin(port, pin_num);
