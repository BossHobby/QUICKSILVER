...
#define INTEGRAL_WINDUP_LIMIT 20.0f
...
static float target_altitude = 10.f;
...
static void nav_update_rth() {
  ...
  const float error = target_altitude - state.altitude;

  static float integral = 0;
  integral = constrain(integral + error * dt, -INTEGRAL_WINDUP_LIMIT, INTEGRAL_WINDUP_LIMIT);

  static float last_error = 0.f;
  const float derivative = (error - last_error) / dt;
  last_error = error;

  const float p_term = KP * error;
  const float i_term = KI * integral;
  const float d_term = KD * derivative;

  state.rx_override.throttle = constrain(p_term + i_term + d_term, MIN_THROTTLE, MAX_THROTTLE);
}
...
void nav_update() {
  ...
  if (!flags.arm_state) {
    ...
    static uint8_t last_rth_state = 0;
    if (rx_aux_on(AUX_RETURN_TO_HOME) != last_rth_state) {
      if (rx_aux_on(AUX_RETURN_TO_HOME)) {
        ...
        target_altitude = state.altitude + 10.f;
      } else {
      ...
      last_rth_state = rx_aux_on(AUX_RETURN_TO_HOME);
    }
