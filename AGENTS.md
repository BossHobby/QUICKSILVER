...
- IO owns RX transport, battery, barometer, utility outputs, GPS and VTX when DisplayPort is absent. With a configured DisplayPort UART, OSD owns VTX service too, so that UART has one sending task; initialize VTX state before starting OSD. Routine IO passes do not take `profile_mutex` and delay one tick between passes. Barometer runs at 10 ms, GPS at 5 ms, and VTX remains ground-only. Flight retains RX conditioning, control, Blackbox capture and cooperative navigation. RX decoders stage channels privately, IO publishes complete frames to a coalescing mailbox, and Flight owns `state.rx_channels`.
