...
- Distinguish the arming gate from synchronization. Disarmed Flight, USB and OSD serialize complete passes with `profile_mutex`, owned and initialized by `core/profile.cpp`. USB can disconnect before a command finishes. Armed Flight never takes this mutex; OSD continues rendering but cannot run gestures or menu maintenance while armed/airborne.
